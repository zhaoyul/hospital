...
* 每次提交代码之前代码格式化
- 使用 ~clj -Ttools install io.github.weavejester/cljfmt '{:git/tag "0.13.1"}' :as cljfmt~ 安装cljfmt
- 使用 ~clj -Tcljfmt fix~ 直接进行格式化, 保证统一的行距, 对齐.
* 每次提交代码之前错误检查
- 安装clj-kondo
- 执行 ~clj-kondo  --lint src test~ 发现并且修复存在的问题, 优先修复error
* 项目结构
  #+BEGIN_EXAMPLE
  hospital
...
* 权限控制
系统通过 `roles`、`permissions` 和 `role_permissions` 三张表实现功能级访问控制。
`users` 表的 `role` 字段与 `roles` 表中的 `name` 对应，决定用户身份。
...
`permissions` 表的 `module` 列表示功能模块，`action` 列表示具体操作。目前内置模
块及其默认操作如下：
| 模块       | action | 说明                     |
|------------+--------+--------------------------|
| 纵览信息   | view   | 首页统计和图表           |
...
若需新增模块或操作，请按以下流程：
1. 创建新的数据库迁移，在 `permissions` 表插入记录。
2. 在 `role_permissions` 表为相应角色添加权限。
3. 同步更新前端的权限树，使其能在“系统管理”页面中配置。
