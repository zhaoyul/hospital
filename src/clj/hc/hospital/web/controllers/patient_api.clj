...
(defn submit-assessment!
  "接收患者端提交的评估数据并存入数据库。数据结构应符合 `PatientSelfAssessmentSpec`，因此无需额外转换。"
  [{{:keys [body]} :parameters :keys [query-fn] :as _request}]
  (log/info "接收到患者评估数据:" body)
  (try
    (let [current-time (str (Instant/now))
          body-with-meta (update body :基本信息
                                 (fn [info]
                                   (-> info
