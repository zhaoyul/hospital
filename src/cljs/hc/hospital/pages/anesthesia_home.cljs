    ...
    (r/create-class
     {:component-did-update
      (fn [this [_ old-medical-history]]
        (when (not= medical-history old-medical-history)
          (.resetFields form))) ; Reset form when data changes externally
      :display-name "brief-medical-history"
      :reagent-render
      (fn []
        [antd/form {:form form
                    :layout "vertical"
    ...
    (r/create-class
     {:component-did-update
      (fn [this [_ old-exam-data]]
        (when (not= exam-data old-exam-data)
          (.resetFields form)))
    ...
    (r/create-class
     {:component-did-update
      (fn [this [_ old-lab-data]]
        (when (not= lab-data old-lab-data)
          (.resetFields form)))
