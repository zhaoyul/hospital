(ns hc.hospital.pages.settings
  (:require
   ...
   ["react" :as React]
   ["antd" :refer [Button Form Input Modal Select Space Space Table Typography
                   Upload]] ; Added Modal, Table, Space
...
(defn system-settings-content []
  (let [doctors @(rf/subscribe [::subs/doctors])
        ...
        [form] (Form.useForm)] ; Ant Design Form instance
    ...
    [:div {:style {:flex 1
     ...
     [:> Typography.Title {:level 2 :style {:marginTop 0 :marginBottom "16px" :fontSize "18px" :fontWeight 500 :color "#333"}} "系统设置"]
     ...
     [:> Table {:dataSource doctors
                ...
                :columns  [{:title "姓名" :dataIndex "name" :key "name" :align "center"}
                           {:title "账号" :dataIndex "username" :key "username" :align "center"}
                           {:title "角色" :dataIndex "role" :key "role" :align "center"}
                           {:title "电子签名" :dataIndex "signature" :key "signature" :align "center"
                            :render (fn [signature _record]
                                      (when signature
                                        (if (or (string? signature) (:thumbUrl signature))
                                          [:img {:src (if (string? signature) signature (:thumbUrl signature))
                                                 :alt "签名"
                                                 :style {:width "48px" :height "48px" :borderRadius "50%" :objectFit "cover" :border "1px solid #eee"}}]
                                          [:span "无签名"])))}
                           {:title "操作" :key "action" :align "center"
                            :render (fn [_text record]
                                      (let [doc (js->clj record :keywordize-keys true)]
                                        [:> Space {}
                                         [:> Button {:type "primary" :ghost true :size "small" :icon (r/as-element [:> icons/EditOutlined])
                                                     :on-click (fn []
                                          ...
                                          "编辑"]
                                         [:> Button {:danger true :ghost true :size "small" :icon (r/as-element [:> icons/DeleteOutlined])
     ...
     (when modal-visible?
       [:> Modal {:title (if (:username editing-doctor) "编辑医生" "新增医生")
        ...
        [:> Form {:form form
         ...
         [:> Form.Item {:name "name" :label "姓名" :rules #js [{:required true :message "请输入姓名!"}]}
          [:> Input {}]]
         [:> Form.Item {:name "username" :label "账号" :rules #js [{:required true :message "请输入账号!"}]}
          [:> Input {:disabled (boolean (:username editing-doctor))}]]
         [:> Form.Item {:name "role" :label "角色" :rules #js [{:required true :message "请选择角色!"}]}
         ...
         [:> Form.Item {:name "signature-file" :label "电子签名" :valuePropName "fileList"}
          [:> Upload {:name "signature"
                      ...
                      :beforeUpload (fn [file]
                                      (let [reader (js/FileReader.)]
                                        (set! (.-onload reader)
                                              #(rf/dispatch [::events/update-editing-doctor-field :signature (.. % -result -target)]))
           ...
           (if (and editing-doctor (:signature editing-doctor) (not (object? (:signature editing-doctor))))
             ...
             [:div [:> icons/PlusOutlined] [:div {:style {:marginTop 8}} "上传"]])]]]])]))
